...
public class LocService extends Service implements AMapLocationListener {
    ...
    public void onLocationChanged(AMapLocation location) {
        ...
        if (location.getCity() == null)
            return;
        F.address =F.locationAddress = location.getProvince()+location.getCity() + location.getDistrict()+ location.getStreet()+location.getStreetNum();
        ...
        Frame.HANDLES.sentAll("FrgCxHome", 1, null);
        mlocationClient.stopLocation();
        mlocationClient.onDestroy();
